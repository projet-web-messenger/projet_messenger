...
import {
  ...
  type UserConversationsQueryVariables,
  UserStatus,
} from "@/gql/graphql";
      ...
      {userConversations.map(({ conversation }) => {
        ...
        const conversationAvatar = !isGroupChat
          ? conversation.users.find(
              (useConversation) => useConversation.user.id !== userId
            )?.user.avatar
          : conversation?.avatar;
        ...
        const conversationDisplayName = !isGroupChat
          ...
          : conversation?.users
              .map((useConversation) =>
                useConversation.user.id === userId
                  ? useConversation.user.displayName
                  : ""
              )
              .filter(Boolean)
        ...
        return (
          <li
            ...
            <Button
              ...
              asChild>
              <Card
                className="h-auto flex-none flex-row items-center justify-start rounded-lg border-0"
                asChild>
                <LinkBox>
                  ...
                  <CardBody className="h-full justify-center p-2">
                    ...
                    <CardTitle className="font-semibold text-md">
                      <LinkOverlay
                        ...
                        asChild>
                        <Link href={`/channels/me/${conversation.id}`}>
                  ...
                  <CardFooter className="h-full items-center justify-between p-2">
                    <CloseButton
                      variant="unstyled"
                      size="sm"
                      className="p-0.5 opacity-50 transition-all hover:scale-105 hover:opacity-100"
                    />
                  </CardFooter>
                </LinkBox>
              </Card>
