...
export default async function FriendArea() {
  ...
  const {
    data: { friendRequests },
  } = await getClient().query<GetFriendsRequestsQuery, GetFriendsRequestsQueryVariables>({
  ...
  return (
    <Tabs defaultValue={defaultValue} className="min-w-0 space-y-4">
      <TabsList as="header" className="flex h-12 items-center gap-4 border-b px-4 py-2">
        <Button as="div" className="h-8 cursor-default rounded-lg px-2" size="md" variant="plain">
          <Icon asChild>
        ...
        {onlineFriends.length > 0 ? (
          <TabsTrigger value="online" asChild>
            <Button as="div" className="h-8 rounded-lg px-2 text-md" size="md" variant="ghost">
              Online
            </Button>
          </TabsTrigger>
        ) : null}
        ...
        <TabsTrigger value="add-friend" asChild>
          <Button as="div" className="h-8 rounded-lg px-2" size="md" variant="solid">
            Add Friend
          </Button>
        </TabsTrigger>
      </TabsList>
      ...
      {onlineFriends.length > 0 ? (
        <TabsContent value="online" className="px-4">
          <InputGroup endElement={<Icon as={LuSearch} />} className="w-full">
  ...
  return (
    <>
      <ul>
        {friends.map((friend) => {
          return (
            <li key={friend.id} className="border-t py-2">
                  ...
                  <Card className="h-auto flex-none flex-row items-start justify-start rounded-lg border-0" asChild>
                    <LinkBox>
                      <CardHeader className="relative justify-center p-2">
                        <Avatar src={friend.avatar ?? undefined} name={friend.displayName ?? undefined} size="md" />
                      ...
                      <CardBody className="h-full justify-center p-2">
                        <CardTitle className="font-medium text-sm">
                          <LinkOverlay as="button" type="submit">
                            {friend.displayName}
                          </LinkOverlay>
                        </CardTitle>
                        <CardDescription className="line-clamp-1 text-ellipsis text-gray-500 text-xs">{friend.status}</CardDescription>
                      </CardBody>
  ...
  return (
    <ul>
      {pendingFriendRequests.map((friendRequest) => {
        ...
        return (
          <li key={sender.id} className="border-t py-2">
            <Button variant="ghost" asChild>
              <Card className="h-auto flex-none flex-row items-start justify-start rounded-lg border-0">
                <CardHeader className="relative justify-center p-2">
                  <Avatar src={sender.avatar ?? undefined} name={sender.displayName ?? undefined} size="md" />
                </CardHeader>
                <CardBody className="h-full justify-center p-2">
                  <CardTitle className="font-medium text-sm">{sender.displayName}</CardTitle>
                  <CardDescription className="line-clamp-1 text-ellipsis text-gray-500 text-xs capitalize">Incoming friend request</CardDescription>
                </CardBody>
